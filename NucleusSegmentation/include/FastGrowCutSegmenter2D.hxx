#ifndef FastGrowCutSegmenter2D_hxx
#define FastGrowCutSegmenter2D_hxx
...
namespace FGC
{
  ...
  ::InitializationAHP()
  {
    ...
    if((m_heap = new FibHeap) == NULL || (m_hpNodes = new HeapNode[m_DIMXY+1]) == NULL)
      {
    ...
    if(!m_bSegInitialized)
      {
        m_imLab.resize(m_DIMXY);
        m_imDist.resize(m_DIMXY);
        m_imLabPre.resize(m_DIMXY);
        m_imDistPre.resize(m_DIMXY);
            ...
            for(iy = -1; iy <= 1; iy++)
              {
                if(!(ix == 0 && iy == 0))
                  {
                    m_indOff.push_back(ix + iy*m_DIMX);
                  }
              }
          }

        // Determine neighborhood size at each vertice
        m_NBSIZE = std::vector<unsigned char>(m_DIMXY, 0);
        ...
        for(index = 0; index < m_DIMXY; index++)
          {
...
//    int pixCenter;
  ...
  ::checkLabelIntegrity()
  {
    for (std::size_t it = 0; it < m_imLab.size(); ++it)
      {
        if (m_imLab[it] > 2000)
          {
            std::cerr<<"it = "<<m_imLab[it]<<std::endl;
            abort();
          }
      }
